...
from network.multiModalityNetworks import NetworkA
...
parser.add_argument('--network', type=str, default='A', metavar='str',
...
parser.add_argument('--gpu', type=str, default='0', metavar='N',
                    help='gpu numbers')
parser.add_argument('--initial_kernel', type=int, default=32, metavar='N',
                    help='input_channel')
...
# The network used
if args.network == 'A':
    model = NetworkA(init_kernel=args.initial_kernel, device=device)
    # pass
if args.network == 'B1':
    pass
if args.network == 'B2':
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    if epoch % 5 == 0 or epoch > args.epochs - 20:
        model.eval().cpu()
        save_filename = 'ADClassifier_B2' + "_epoch_" + str(epoch) + ".model"
